...
export type InterviewItem = {
  slug: string;
  title: string;
  thumbnail?: string; // 画像URL（省略時はプレースホルダー）
  ...
  return (
    <section className="mx-auto max-w-screen-xl px-4 py-4">
      <div className="mb-4 flex items-end justify-between">
        ...
        <Link href="/topics" className="text-sm font-medium text-neutral-600 hover:text-neutral-900">
          すべて見る →
        </Link>
      </div>
...
function GuideCard({ item }: { item: InterviewItem }) {
  const img = item.thumbnail || "/images/placeholder-16x9.jpg";

...
export type PageProps = ServiceCardProps & {
...
function Page({
  ...
  return (
    <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 space-y-2">
      {/* メインカード */}
...
// =====================
export default function SugarDaddyComponent() {
  return (
    <Page
      rank={1}
      name="SugarDaddy（シュガーダディ）"
      bannerUrl="https://lastdecade.s3.ap-northeast-1.amazonaws.com/app/sugardaddy/SugarDaddy_thumbnail.png"
      memberCount="非公開"
      pricing={{
        female: "完全無料",
        male: "1ヶ月 8,000円〜",
      }}
      ageRange={{ female: "20代前半〜30代前半", male: "30代前半〜50代半ば" }}
      incomeProof="あり"
      features={[
        "国内最大級のパパ活サイト",
        "食事だけでOKなパパが多い",
      ...
      company="シナプスコンサルティング株式会社"
      officialUrl="https://sugardaddy.jp/"
      lead={[
        "シュガーダディは、パパ活女子の中では定番のパパ活アプリです。パパ活専用サービスとしては老舗で、パパ活の趣旨を理解している良パパが多いため安心して使えます。",
        "パパの年齢層は若干高めなので、食事のみの優しいパパと出会いやすいです。相手の通報回数が見れるため、トラブルになりそうな会員を避けて出会えます。パパ活が初めての女性でも、安全にパパを見つけやすいでしょう。",
        "また、マッチング不要でやり取りできるのも嬉しいポイント。自分からアプローチすればチャンスを増やせるため、他のパパ活アプリでマッチングが少なかった女性にもおすすめです。",
      ]}
      reasons={[
      ...
      bestFor={[
        "安心できる大手のサービスでパパ活したい女性",
        "地方でパパ活をしていてなかなか良パパが掴めない女性",
        "食事だけのパパを探しているコツコツ稼ぎたい女性",
      ]}
      ctaText="シュガーダディを無料ではじめる"
      ctaHref="https://lovean.jp/"
    />
  );
}
